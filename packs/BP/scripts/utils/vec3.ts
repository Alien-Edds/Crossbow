...
var Vector3Utils = class _Vector3Utils {
   ...
   */
  static toString(v: Vector3, options?: {decimals?: number, delimiter?: string}) {
...
var Vector2Utils = class {
   ...
   */
  static toString(v: Vector2, options?: {decimals?: number, delimiter?: string}) {
    const decimals = options?.decimals ?? 2;
...
var Vector3Builder = class implements Vector3 {
  x: number;
  y: number;
  z: number;
  constructor(first: Vector3 | string | number, second: number, z?: number) {
   ...
   */
  toString(options: {decimals?: number, delimiter?: string}) {
   ...
   * Clamps the components of a vector to limits to produce a new vector
   */
  clamp(limits: {min?: Vector3, max?: Vector3}) {
...
var Vector2Builder = class {
  x;
  y;
  constructor(first: Vector3 | string | number, second: number) {
    if (typeof first === "object") {
      this.x = first.x;
      this.y = first.y;
    } else if (typeof first === "string") {
